...
void encode64 (char *orig, char *dest, int nbcar){
  ...
  static char   valcar [] = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
...
int decode64(char *buffer) {
  ...
  while(buffer[posorig]>' ' && buffer[posorig]!='=') {
    ...
    for(i=0; i<4 && buffer[posorig]!='='; i++) {
      ...
      if ('A' <= car && car <= 'Z') valcar[i]=car-'A';
      else if('a' <= car && car <= 'z') valcar[i]=car+26-'a';
      else if('0' <= car && car <= '9') valcar[i]=car+52-'0';
      else if(car == '+') valcar[i]=62;
      else if(car == '/') valcar[i]=63;
    }
